...
void draw_buttons() {
    ...
    char* msg = (char*) "Command Console Buttons";
    move((LINES - 2 - BTN_SIZE_Y - 8) / 2, (COLS - strlen(msg)) / 2);
    attron(A_BOLD);
    printw(msg);
    attroff(A_BOLD);

    draw_btn(vx_decr_btn, (char*)"Vx-", 1);
    draw_btn(vx_stp_button, (char*)"STP", 2);
    draw_btn(vx_incr_btn, (char*)"Vx+", 3);

    draw_btn(vz_decr_btn, (char*)"Vz-", 1);
    draw_btn(vz_stp_button, (char*)"STP", 2);
    draw_btn(vz_incr_btn, (char*)"Vz+", 3);
}
