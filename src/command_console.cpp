...
void Stop(int num , siginfo_t* info, void* vp){
    ...
    if(ReadPID(M1F) == info->si_pid or ReadPID(InspectF) == info->si_pid){
        PrintLog("Emergency Stop for x-axis...\n");//this will be printed into the log file
    ...
    if (ReadPID(M2F) == info->si_pid or ReadPID(InspectF) == info->si_pid){
        PrintLog("Emergency Stop for y-axis...\r");//this will be printed into the log file
        // if it is from M1 means that the hoist hitted the limits and we should 
        // update the no hit variable
...
void Rewind(int num){
    ...
    //while the hoist didn't hit the limit of both axis keep repeating sleep(2);
    //TODO: Usinterruptable Rewind
